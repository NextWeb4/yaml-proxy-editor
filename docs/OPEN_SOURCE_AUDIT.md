...
### 2026-07-15 单网站分流方案审计

| 方案名称 | 来源 | 许可证 | 核心能力 | 优点 | 缺点 | 维护状态 | 与当前项目契合度 | 可能冲突点 | 是否采用 | 采用方式 |
|---|---|---|---|---|---|---|---|---|---|---|
| 现有 `ruleEditor` + 标准 `URL` + `yaml` | 当前代码、Web 标准、npm `yaml` | 平台 API / ISC | URL hostname 规范化、规则顺序写回 | 无新增体积、离线可用、复用现有 MATCH/去重链路 | 不做公共后缀推断 | 随浏览器与当前依赖维护 | 高 | 无网络、架构或许可证冲突 | 采用 | `ruleEditor.ts` 负责规范化和写回，页面只收集表单 |
| `tldts@7.4.8` | npm / GitHub | MIT | 公共后缀、注册域和子域解析 | 复杂域名识别成熟 | 当前需求不需要推断注册域；新增直接依赖和维护面 | 活跃，npm 2026-07-09 更新 | 中 | 与标准 `URL` 的 hostname 能力部分重复 | 不采用 | 保持现有传递依赖不变，不声明直接依赖 |
| `validator@13.15.35` | npm / GitHub | MIT | 通用 URL/域名校验 | 校验器成熟、覆盖面广 | 能力过宽，增加包体和 API 面 | 活跃，npm 2026-04-02 更新 | 低 | 与标准 `URL` 和本地校验重复 | 不采用 | 不引入 |

采用范围：直接复用标准 `URL` 提取 hostname、现有 `yaml` 解析/序列化和 `ruleEditor` 的 MATCH 归一化。只借鉴 Clash/Mihomo 按顺序命中规则的设计语义；不采用公共后缀推断、联网探测或新的校验依赖。现有高级规则、批量导入、模板、注释和保存链路全部保留。

冲突检查：不改变 Tauri/Rust、目录结构、运行/构建命令、配置系统、权限模型或应用联网边界；无数据库影响；许可证和离线要求无新增风险。回滚时删除网站表单与 `upsertWebsiteRuleInYaml`，原 `addRuleToYaml` 等接口不受影响。
